...
export function ResultsScreen() {
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 md:p-6 space-y-6">
      ...
      <div className={`rounded-xl shadow-lg border p-6 md:p-8 text-white ${
        isPassed
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className={`px-2.5 py-0.5 rounded text-xs font-bold uppercase tracking-wider ${
                isPassed ? 'bg-emerald-500/30 text-emerald-200 border border-emerald-400/40' : 'bg-red-500/30 text-red-200 border border-red-400/40'
              }`}>
                {isPassed ? 'Official Result: Passed' : 'Official Result: Failed'}
              </span>
            </div>
            ...
            <p className="text-xs text-slate-300 mt-1">
              Passing threshold: {passingThresholdPercent}% ({minPassingCount} of {totalQuestions} required)
            </p>
          </div>
            ...
            <div className="space-y-6">
              {incorrectQuestions.length === 0 ? (
                <div className="text-center py-12 text-slate-500">
                  ...
                  <p className="font-semibold text-slate-800">Perfect Score!</p>
                  <p className="text-xs text-slate-500">You did not miss any questions in this session.</p>
                </div>
              ) : (
                incorrectQuestions.map((q, idx) => (
                  <div key={q.id} className="border border-red-200 rounded-lg p-5 bg-red-50/20 space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-slate-500 font-medium uppercase">
                          {q.source === 'generated' ? 'Generated Question' : 'Source File'}
                        </span>
                      </div>
                    ...
                    <div className="space-y-1 text-xs pt-2 border-t border-slate-200/80">
                      {q.options.map((opt) => {
                        const isUserPicked = q.userAnswers.includes(opt.letter);
                        const isCorrectOpt = q.correctLetters.includes(opt.letter);
                        ...
                        return (
                          ...
                          >
                            <span className="font-bold font-mono uppercase">{opt.letter}.</span>
                            <span>{opt.text}</span>
                          </div>
                        );
                      })}
                    ...
                    <div className="p-3.5 bg-white border border-slate-200 rounded-lg text-xs leading-relaxed text-slate-700">
                      <span className="font-bold text-slate-900 block mb-1">Explanation & Rationale:</span>
                      {q.explanation}
                    </div>
                  </div>
                ))
          ...
          {activeTab === 'correct' && (
            <div className="space-y-6">
              {correctQuestions.length === 0 ? (
                <div className="text-center py-12 text-slate-500">
                  <p className="font-semibold text-slate-800">No correct answers in this session.</p>
                </div>
              ) : (
                correctQuestions.map((q) => (
                  <div key={q.id} className="border border-emerald-200 rounded-lg p-5 bg-emerald-50/20 space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {categoryStats.map((cat) => (
                  <div key={cat.id} className="border border-slate-200 rounded-lg p-4 bg-slate-50">
                    ...
                    <div className="w-full bg-slate-200 rounded-full h-2 overflow-hidden mt-2">
                      <div
                        className={`h-2 rounded-full transition-all duration-500 ${
                          cat.percentage >= 65 ? 'bg-emerald-600' : 'bg-rose-600'
                        }`}
          ...
          {activeTab === 'generated' && (
            <div className="space-y-6">
              <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-900 leading-relaxed">
                <strong>Sourcing Audit:</strong> The questions below were automatically generated by the fallback engine to meet official ADM-201 category blueprint weighting requirements where the input file did not have enough questions.
              </div>

              {generatedQuestions.length === 0 ? (
                <div className="text-center py-8 text-slate-500 text-xs">
                  All 60 questions were sourced directly from your uploaded file (0 generated fallback questions).
                </div>
              ) : (
                generatedQuestions.map((q) => (
                  <div key={q.id} className="border border-amber-200 rounded-lg p-4 bg-amber-50/30 space-y-3 text-xs">
                    <div className="flex items-center justify-between">
                    ...
                    <div className="text-slate-700">
                      <strong>Correct Answer:</strong> <span className="font-mono">{q.correctAnswerString}</span>
                    </div>
