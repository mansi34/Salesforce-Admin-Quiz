...
export function PreExamScreen() {
  ...
  const totalGenerated = breakdownList.reduce(
    (acc, c) => acc + c.generatedCount,
    0,
  );
  const totalFileSourced = breakdownList.reduce(
    (acc, c) => acc + c.fileCount,
    0,
  );

  const unreadableCount = allParsedQuestions.filter(
    (q) => q.isGradable === false,
  ).length;
  const usableCount = allParsedQuestions.length - unreadableCount;
          ...
          <div className='flex flex-col md:flex-row md:items-center justify-between gap-4'>
            <div>
              ...
              <p className='text-xs text-slate-400 mt-0.5'>
                Parsed question pool:{' '}
                <span className='text-white font-semibold'>
                  {usableCount}
                </span>{' '}
                questions loaded from source file
              </p>
              {unreadableCount > 0 && (
                <p className='text-xs text-amber-300/90 mt-1'>
                  {unreadableCount} question
                  {unreadableCount === 1 ? ' was' : 's were'} skipped
                  because the answer could not be read from the file.
                  Skipped questions are never asked, so they can never
                  be marked wrong.
                </p>
              )}
            </div>
        ...
        <div className='p-6 md:p-8 space-y-8'>
          ...
          <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
            ...
            <div className='bg-slate-50 border border-slate-200 rounded-lg p-4'>
              ...
              <span className='text-[11px] text-slate-400'>
                {EXAM_CONFIG.MIN_PASSING_SCORE} of 60 to pass
              </span>
            </div>
          ...
          <div className='bg-slate-50 border border-slate-200 rounded-lg p-5 space-y-3'>
            ...
            <ul className='text-xs text-slate-600 space-y-2 list-disc list-inside leading-relaxed'>
              <li>
                <strong>Single-Direction Progression:</strong>{' '}
                Questions are presented one at a time. No backward
                navigation or review is permitted until final
                submission.
              </li>
              <li>
                <strong>No In-Exam Feedback:</strong> No hints,
                instantaneous scoring, or answer validation are
                revealed during the session.
              </li>
              <li>
                <strong>Strict Pause Policy:</strong> You are granted
                exactly <strong>one pause</strong> for the entire
                exam. Resuming the exam permanently uses this
                allowance.
              </li>
              <li>
                <strong>Continuous Timer:</strong> The 105-minute
                countdown runs uninterrupted. If the timer reaches
                00:00, the exam is automatically submitted for
                grading.
              </li>
            </ul>
            ...
            <div className='pt-3 border-t border-slate-200'>
              <label className='flex items-center gap-3 cursor-pointer select-none'>
                <input
                  ...
                  onChange={(e) =>
                    setConfirmedRules(e.target.checked)
                  }
                  className='w-4 h-4 text-blue-600 rounded border-slate-300 focus:ring-blue-500'
                />
                <span className='text-xs font-semibold text-slate-800'>
                  I acknowledge the examination rules and understand
                  that timing and single-pause enforcement are final.
                </span>
              </label>
            </div>
          </div>
